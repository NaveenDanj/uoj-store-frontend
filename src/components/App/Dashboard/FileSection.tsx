...
interface FileData {
    ...
    file_size: number; // Size in KB
}

// FileItem component to display each file
        ...
        <div className="flex cursor-pointer w-full flex-row lg:flex-col justify-between p-3 border border-black/1 dark:border-white/1 rounded-lg">
            <div className='flex lg:flex-col gap-3'>
                ...
                <div className='flex flex-col'>
                    <label className="my-auto text-md text-start lg:text-center font-medium cursor-pointer">{original_name}</label>
...
// FileSection component to display all file items
    ...
    useEffect(() => {
        const fetchLargestFiles = async () => {
            try {
                ...
                const filesData: FileData[] = response.data.top_files; // Adjust this based on your actual response structure
        ...
        <div className="flex flex-col w-full mt-10">
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-4 sm:grid-cols-3 gap-4 mt-5 w-full">
                {largestFiles.map((file) => (
                    <FileItem
                        key={file.file_id} // Ensure you use a unique key
