    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <div className="w-full flex flex-col  md:flex-row justify-between mt-8 pl-3">
                ...
                <div className="flex flex-row gap-4 px-2 my-auto">
                    {selectedItem.length > 0 && (
                        <Button onClick={restoreSelected} className="">
                            Restore Selected
                        </Button>
                    )}
            ...
            <div className="mt-10 pl-3 grid grid-cols-2 gap-4 mb-8 sm:grid-cols-3 md:grid-cols-4 2xl:grid-cols-7 3xl:grid-cols-8">
                {folderList.map((item, index) => (<FolderItem isTrash="trash" selectedItem={selectedItem} setSelectedItem={setSelectedItem} folderStack={[]} folder={item} key={index} setFolderStack={function (value: SetStateAction<{ id: number; name: string; }[]>): void {
                    throw new Error("Function not implemented.");
                }} />))}
                {fileList.map((item, index) => (<FileItem isTrash="trash" selectedItem={selectedItem} setSelectedItem={setSelectedItem} key={index} file={item} />))}
            </div>
